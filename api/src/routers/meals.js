  ...
  try {
    ...
    if ("maxPrice" in req.query) {
      ...
      const data7 = await knex.raw(
        `SELECT m.id, m.title, m.description, m.location, m.when, m.max_reservations, m.price, m.created_date, COALESCE(CAST(m.max_reservations AS INTEGER) - SUM(CAST(_reservation.number_of_guests AS INTEGER)), CAST(m.max_reservations AS INTEGER)) AS available_reservations, m.image_url FROM _meal AS m LEFT JOIN _reservation ON m.id::integer=reservation.meal_id::integer GROUP BY m.id, m.title, m.description, m.location, m.when, m.max_reservations, m.price, m.created_date, m.image_url ORDER BY m.id ASC LIMIT CAST(${limit} AS INTEGER);`
      );
